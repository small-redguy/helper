...
var OtherKey =
	''; //第三账号或以上的Cookie json串数据, 以下样例为第三第四账号：var OtherKey = '[{"cookie":"pt_key=xxxxxx;pt_pin=yyyyyy"},{"cookie":"pt_key=xxxxxx;pt_pin=yyyyyy"}]'


var LogDetails = false; //是否开启响应日志, true则开启

var stop =
	'0'; //自定义延迟签到, 单位毫秒. 默认分批并发无延迟; 该参数接受随机或指定延迟(例: '2000'则表示延迟2秒; '2000-5000'则表示延迟最小2秒,最大5秒内的随机延迟), 如填入延迟则切换顺序签到(耗时较长), Surge用户请注意在SurgeUI界面调整脚本超时; 注: 该参数Node.js或JSbox环境下已配置数据持久化, 留空(var stop = '')即可清除.

var DeleteCookie = false; //是否清除所有Cookie, true则开启.

var boxdis =
	true; //是否开启自动禁用, false则关闭. 脚本运行崩溃时(如VPN断连), 下次运行时将自动禁用相关崩溃接口(仅部分接口启用), 崩溃时可能会误禁用正常接口. (该选项仅适用于QX,Surge,Loon)
...
$.get = (o, f) => {
	if (o['headers']) {
		...
		o['headers']['User-Agent'] = 'JD4iPhone/167169 (iPhone; iOS 13.4.1; Scale/3.00)'
	}
	that.get(o, f)
}
$.post = (o, f) => {
...
function notify() {
	return new Promise(resolve => {
		try {
			...
			var beans = merge.TotalBean && merge.TotalBean.Qbear ?
				`${merge.TotalBean.Qbear}京豆${Steel?`, `:``}` : ""
			var Money = merge.TotalMoney && merge.TotalMoney.TMoney ?
				`${merge.TotalMoney.TMoney}现金${Cash?`, `:``}` : ""
			var Subsidy = merge.TotalSubsidy && merge.TotalSubsidy.TSubsidy ?
			...
			const numFix = !Key && !DualKey ? DualAccount - 2 : Key && DualKey ? DualAccount : DualAccount -
				1 || DualAccount
			...
			const disa = !notify || amount ?
				`【温馨提示】:  检测到${$nobyda.disable?`上次执行意外崩溃, `:``}已禁用${notify?`${amount}个`:`所有`}接口, 如需开启请前往BoxJs或查看脚本内第100行注释.\n` :
				``
			$nobyda.notify("", "", Name + one + two + three + four + five + disa + notify, {
				'media-url': $nobyda.headUrl ||
					'https://cdn.jsdelivr.net/gh/NobyDa/mini@master/Color/jd.png'
			});
			$nobyda.headUrl = null;
			if ($nobyda.isJSBox) {
				Shortcut = (typeof(Shortcut) == 'undefined' ? '' : Shortcut) + Name + one + two + three + four +
					five + "\n"
			}
			double();
		} catch (eor) {
...
async function ReadCookie() {
	...
	if (KEY || OtherKey) {
		...
		var ocks =  [];
		if(OtherKey){
		    ocks=JSON.parse(OtherKey) ||[];
		}
		let cookieArr = ocks.map(item => item.cookie)
		if(KEY){
		    cookieArr.push(KEY);
		}
					...
					if (error) {
						...
						if (cc.code == 3) {
							...
							if (data.match(/dailyAward/)) {
								merge.JDBean.notify = "京东商城-京豆: 成功, 明细: " + cc.data.dailyAward
									.beanAward.beanCount + "京豆 🐶"
								merge.JDBean.bean = cc.data.dailyAward.beanAward.beanCount
							} else if (data.match(/continuityAward/)) {
								merge.JDBean.notify = "京东商城-京豆: 成功, 明细: " + cc.data
									.continuityAward.beanAward.beanCount + "京豆 🐶"
								merge.JDBean.bean = cc.data.continuityAward.beanAward.beanCount
							} else if (data.match(/新人签到/)) {
								const quantity = data.match(/beanCount\":\"(\d+)\".+今天/)
								merge.JDBean.bean = quantity ? quantity[1] : 0
								merge.JDBean.notify = "京东商城-京豆: 成功, 明细: " + (quantity ?
									quantity[1] : "无") + "京豆 🐶"
							} else {
			...
			$nobyda.post(JRSUrl, function(error, response, data) {
				try {
					...
					if (data.match(/\"resBusiCode\":0/)) {
						...
						merge.JRSteel.steel = leng ? leng > 9 ? `0.${leng}` : `0.0${leng}` :
							spare ? spare : 0
						merge.JRSteel.notify =
							`京东金融-钢镚: 成功, 明细: ${merge.JRSteel.steel || `无`}钢镚 💰`
...
function filterByPre(body) {
	try {
		var od = JSON.parse(body);
		var params = (od.floatLayerList || []).filter(o => o.params && o.params.match(/enActK/)).map(o => o.params)
		.pop() || (od.floorList || []).filter(o => o.template == "signIn" && o.signInfos && o.signInfos.params && o
			.signInfos.params.match(/enActK/)).map(o => o.signInfos && o.signInfos.params).pop();
		var tId = (od.floorList || []).filter(o => o.boardParams && o.boardParams.turnTableId).map(o => o.boardParams
			.turnTableId).pop();
		var page = od.paginationFlrs;
...
function JDUserSignPre1(s, key, title, acData, ask) {
	return new Promise((resolve, reject) => {
		...
		$nobyda.post(JDUrl, async function(error, response, data) {
			try {
				if (error) {
					...
					data=filterByPre(data);
					const od = JSON.parse(data || '{}');
					const turnTableId = od.qxTid || (od.floorList || []).filter(o => o
						.boardParams && o.boardParams.turnTableId).map(o => o.boardParams
						.turnTableId).pop();
					const page = od.qxPage || od.paginationFlrs;
					if (data.match(/enActK/)) { // 含有签到活动数据
						let params = od.qxAct || (od.floatLayerList || []).filter(o => o.params && o
							.params.match(/enActK/)).map(o => o.params).pop()
						if (!params) { // 第一处找到签到所需数据
							// floatLayerList未找到签到所需数据，从floorList中查找
							let signInfo = (od.floorList || []).filter(o => o.template ==
									'signIn' && o.signInfos && o.signInfos.params && o.signInfos
									.params.match(/enActK/))
		...
		disable(key, title, 2)
		if (typeof(data) == "object") return JDUserSign1(s, key, title, encodeURIComponent(JSON.stringify(
			data)));
				...
				try {
					if (error) {
						...
						if (data.match(/签到成功/)) {
							...
							merge[key].notify =
								`${title}: 成功, 明细: ${merge[key].bean || '无'}京豆 🐶`
							merge[key].success = 1
						} else {
				...
				try {
					if (error) {
						...
						if (cc.result && cc.result.code == 0) {
							...
							merge.JDFSale.notify = "京东商城-闪购: 成功, 明细: " + (merge.JDFSale
								.bean || "无") + "京豆 🐶"
				...
				try {
					if (error) {
						...
						if (cc.result.code == 0) {
							merge.JDFSale.success = 1
							merge.JDFSale.bean = cc.result.jdBeanNum || 0
							merge.JDFSale.notify = "京东闪购-瓜分: 成功, 明细: " + (merge.JDFSale.bean ||
								"无") + "京豆 🐶"
							that.log("\n" + "京东闪购-瓜分签到成功 " + Details)
						} else {
							merge.JDFSale.fail = 1
				...
				try {
					if (error) {
						...
						if (cc.busiCode == "0") {
							...
							merge.JDCash.notify =
								`京东现金-红包: 成功, 明细: ${merge.JDCash.Cash || `无`}红包 🧧`
						} else {
							that.log("\n" + "京东现金-红包签到失败 " + Details)
							merge.JDCash.fail = 1
					...
					if (error) {
						...
						if (data.match(/(\"name\":)/)) {
							...
							if (data.match(/(\"name\":\"京豆\")/)) {
								...
								merge.JDCube.notify =
									`京东商城-魔方: 成功, 明细: ${merge.JDCube.bean || `无`}京豆 🐶`
							} else {
								merge.JDCube.notify =
									`京东商城-魔方: 成功, 明细: ${cc.result.lotteryInfo.name || `未知`} 🎉`
							}
						} else {
							that.log("\n" + "京东商城-魔方签到失败 " + Details)
							merge.JDCube.fail = 1
			...
			$nobyda.get(GameUrl, async function(error, response, data) {
				try {
					if (error) {
						...
						if (!id) {
							var status = $nobyda.ItemIsUsed ? "再次检查" : merge.SpeedUp
								.notify ? "查询本次" : "查询上次"
						...
						if (cc.message == "not login") {
							...
							if (cc.data.task_status == 0 && cc.data.source_id) {
								if ($nobyda.ItemIsUsed) { //如果使用道具后再次开始任务, 则收到奖励
									...
									merge.SpeedUp.notify =
										`京东天天-加速: 成功, 明细: ${merge.SpeedUp.bean || `无`}京豆 🐶`
								}
								await JingDongSpeedUp(s, cc.data.source_id)
							} else if (cc.data.task_status == 1) {
								const percent = Math.round((cc.data.done_distance / cc.data
									.distance) * 100)
								that.log(
								...
								if (step4 && $nobyda.ItemIsUsed) { //如果使用了道具, 则再次检查任务
									...
									if (!merge.SpeedUp.notify) {
										merge.SpeedUp.fail = 1
										merge.SpeedUp.notify =
											`京东天天-加速: 失败, 加速中${percent<10?`  `:``}[${percent}%] ⚠️`
									}
								}
							...
							if (data.match(/领过此任务/)) { //处理任务频繁问题
								if (!$nobyda.tryAgain || $nobyda.tryAgain < 3) { //避免死循环
									$nobyda.tryAgain ? $nobyda.tryAgain += 1 : $nobyda
										.tryAgain = 1
					...
					$nobyda.get({
						...
						try {
							...
							if (list.length == spaceEventCount) {
								if (list.length == spaceNumTask) $nobyda.isAllEvents =
									true; //避免重复查询
		...
		await new Promise(resolve => {
			...
			if (CID) {
				...
				for (var i = 0; i < CID.length; i++) {
					const TUrl = {
						url: 'https://api.m.jd.com/?appid=memberTaskCenter&functionId=energyProp_gain&body=%7B%22source%22%3A%22game%22%2C%22energy_id%22%3A' +
							CID[i] + '%7D',
						headers: {
							Referer: 'https://h5.m.jd.com/babelDiy/Zeus/6yCQo2eDJPbyPXrC3eMCtMWZ9ey/index.html',
							Cookie: KEY
					...
					$nobyda.get(TUrl, function(error, response, data) {
						try {
							...
							if (CID.length == count) {
								if (CID.length == NumTask) $nobyda.isAlltasks =
									true; //避免重复查询
					...
					if (error) {
						...
						if (cc.resultCode == 0 && cc.resultData.data.thisAmount) {
							...
							merge.subsidy.notify =
								`京东商城-金贴: 成功, 明细: ${merge.subsidy.subsidy||`无`}金贴 💰`
					...
					if (error) {
						...
						if (cc.resultCode == 0) {
							if (cc.resultData.data.businessData != null) {
								...
								if (cc.resultData.data.businessData.pickStatus == 2) {
									if (data.match(/\"rewardPrice\":\"\d.*?\"/)) {
										...
										const JRDoll_type = data.match(
											/\"rewardName\":\"金贴奖励\"/) ? true : false
										await JingRongDoll(s, key, title, code, '4',
											JRDoll_bean, JRDoll_type)
									} else {
										merge[key].success = 1
									...
									if (!data.match(/"businessCode":"30\dss?q"/)) {
										...
										if (code == 'F68B2C3E71' && belong == 'xianjin') {
											merge[key].Money = ct ? ct[1] > 9 ?
												`0.${ct[1]}` : `0.0${ct[1]}` : 0
											merge[key].notify =
												`${title}: 成功, 明细: ${merge[key].Money||`无`}现金 💰`
										} else if (code == 'F68B2C3E71' && belong ==
											'jingdou') {
											...
											merge[key].notify =
												`${title}: 成功, 明细: ${merge[key].subsidy||`无`}金贴 💰`
										}
									} else {
										const es = cc.resultData.data.businessMsg
										const ep = cc.resultData.data.businessData
											.businessMsg
										const tp = data.match(/已领取|300ss?q/) ? `已签过` :
								...
								merge[key].success = 1
								merge[key].notify =
									`${title}: 成功, 明细: ${(award?num:merge[key].bean)||`无`}${award?`金贴 💰`:`京豆 🐶`}`
							} else {
								that.log(`\n${title}领取异常 ${Details}`)
								if (num) that.log(
							...
							that.log(`\n${title}签到失败 ${Details}`)
							const redata = typeof(cc.resultData) == 'string' ? cc
								.resultData : ''
							merge[key].notify =
								`${title}: 失败, ${cc.resultCode==3?`原因: Cookie失效‼️`:`${redata||'原因: 未知 ⚠️'}`}`
							merge[key].fail = 1;
						}
					}
				} catch (eor) {
					...
					if (error) {
						...
						if (cc.data.success && cc.data.result) {
							...
							merge.JDGetCash.notify =
								`京东商城-现金: 成功, 明细: ${cc.data.result.signCash||`无`}现金 💰`
						} else {
							that.log("\n" + "京东商城-现金签到失败 " + Details)
			...
			$nobyda.get({
				...
				try {
					...
					if (cc.data && cc.data.success === true && cc.data.bizCode === 0) {
						...
						merge.JDGStore.notify =
							`京东商城-超市: 成功, 明细: ${merge.JDGStore.bean||`无`}京豆 🐶`
					} else {
						if (!cc.data) cc.data = {}
						that.log(`\n京东商城-超市签到失败 ${Details}`)
						const tp = cc.data.bizCode == 811 ? `已签过` : cc.data.bizCode == 300 ?
							`Cookie失效` : `${cc.data.bizMsg||`未知`}`
						merge.JDGStore.notify =
							`京东商城-超市: 失败, 原因: ${tp}${cc.data.bizCode==300?`‼️`:` ⚠️`}`
			...
			$nobyda.post({
				...
				try {
					...
					if (cc.msg == 'success' && cc.subCode == 0) {
						...
						merge.JDSecKill.notify =
							`京东秒杀-红包: 成功, 明细: ${merge.JDSecKill.Cash||`无`}红包 🧧`;
					} else {
						that.log(`\n京东秒杀-红包签到失败 ${Details}`);
						merge.JDSecKill.fail = 1;
						merge.JDSecKill.notify =
							`京东秒杀-红包: 失败, ${cc.subCode==103?`原因: 已领取`:cc.msg?cc.msg:`原因: 未知`} ⚠️`;
					}
				} catch (eor) {
			...
			$nobyda.get({
				...
				try {
					...
					if (cc.status && cc.data && cc.data.firstLoginStatus) {
						...
						const qt = cc.status && cc.data && cc.data.firstLoginStatus === false ?
							`已签过` : cc.error && cc.error.code == 2000 ? `Cookie失效` : cc.error &&
							cc.error.msg ? cc.error.msg : `未知`
						merge.JDBuyCar.notify =
							`京东商城-汽车: 失败, 原因: ${qt}${cc.error&&cc.error.code==2000?`‼️`:` ⚠️`}`
			...
			$nobyda.post({
				...
				try {
					...
					if (cc.status === true) {
						...
						merge.JDBuyCar.bean = cc.data && cc.data.beanNum ? cc.data.beanNum :
							0
						merge.JDBuyCar.notify =
							`京东商城-汽车: 成功, 明细: ${merge.JDBuyCar.bean||`无`}京豆 🐶`;
					} else {
						that.log(`\n京东商城-汽车签到失败 ${Details}`);
						merge.JDBuyCar.fail = 1;
						merge.JDBuyCar.notify =
							`京东商城-汽车: 失败, ${cc.error&&cc.error.msg?cc.error.msg:`原因: 未知`} ⚠️`;
					}
				} catch (eor) {
	...
	try {
		if ($request.method != 'OPTIONS' && $request.headers && $request.url !== 'http://www.apple.com/') {
			...
			if (!acObj.cookie) {
				...
				let oldCks = [];
				try {
					oldCks = (ocks && JSON.parse(ocks).map(item => item.cookie)) || [];
				} catch (e) {
					that.log(`写入京东Cookie时转换京东扩展账号数据CookiesJD异常，扩展账号信息：\n${ocks}`)
					oldCks = [];
				}
				...
				if (status) {
					if (status > 0) {
						...
						$nobyda.notify(`用户名: ${acObj.userName}`, ``,
							`${status==2?`更新`:`写入`}京东 [账号${seatNo+1}] Cookie${WT?`成功 🎉`:`失败 ‼️`}`)
					} else {
