 	...
 	for (let codeItem of codeList) {
 		...
 		for (let i = 0; i < cookiesArr.length; i++) {
 			if (cookiesArr[i]) {
 				...
 				let nowTs = new Date().getTime() + new Date().getTimezoneOffset() * 60 * 1000 + 8 * 60 * 60 *
 					1000
 				...
 				if (err) {
 					...
 					if (safeGet(data)) {
 						...
 						if (data.data && data.data.iconArea) {
 							that.log(data.data.iconArea.filter(vo => vo['type'] ===
 									'anchor_darw_lottery').length &&
 								data.data.iconArea.filter(vo => vo['type'] ===
 									'anchor_darw_lottery')[0].data.lotteryId)
 							let act = data.data.iconArea.filter(vo => vo['type'] ===
 								"platform_red_packege_rain")[0]
 							if (act) {
 		...
 		const options = {
 			...
 			"headers": {
 				...
 				"User-Agent": $.isNode() ? (process.env.JD_USER_AGENT ? process.env.JD_USER_AGENT :
 					"JD4iPhone/9.3.5 CFNetwork/1209 Darwin/20.2.0") : ($.getdata('JDUA') ? $
 					.getdata('JDUA') : "JD4iPhone/9.3.5 CFNetwork/1209 Darwin/20.2.0")
 			}
 		}
