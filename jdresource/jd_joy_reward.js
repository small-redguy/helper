...
!(async () => {
  ...
  for (let i = 0; i < cookiesArr.length; i++) {
    if (cookiesArr[i]) {
      ...
      // that.log(`本地时间与京东服务器时间差(毫秒)：${await get_diff_time()}`);
      await joyReward();
    }
  }
...
async function joyReward() {
  try {
    ...
    if ($.getExchangeRewardsRes && $.getExchangeRewardsRes.success) {
      // that.log('success', $.getExchangeRewardsRes);
      ...
      // that.log(`当前积分 ${data.coin}\n`);
      // that.log(`宠物等级 ${data.level}\n`);
      ...
      for (let item of data[giftSaleInfos]) {
        that.log(`${item['giftName']}当前库存:${item['leftStock']}，id：${item.id}`)
      ...
      // that.log(`${giftValue}京豆当前京豆库存:${leftStock}`)
      // that.log(`saleInfoId:${saleInfoId}`)
      // 兼容之前BoxJs兑换设置的数据
      if (rewardNum && (rewardNum === 1 || rewardNum === 20 || rewardNum === 50 || rewardNum === 100 || rewardNum === 500 || rewardNum === 1000)) {
        ...
        //   if (leftStock) {
            ...
            while(count<100){
                ...
                if ($.exchangeRes && $.exchangeRes.success) {
                  if ($.exchangeRes.errorCode === 'buy_success') {
                    // that.log(`兑换${giftValue}成功,【宠物等级】${data.level}\n【消耗积分】${salePrice}个\n【剩余积分】${data.coin - salePrice}个\n`)
            ...
            // that.log(`当前账户积分:${data.coin}\n当前京豆库存:${leftStock}\n满足兑换条件,开始为您兑换京豆\n`);
            // that.log(`\n您设置的兑换${giftValue}京豆库存充足,开始为您兑换${giftValue}京豆\n`);
        //   } else {
        //     that.log(`\n按您设置的兑换${rewardNum}京豆失败，原因：京豆库存不足，已抢完，请下一场再兑换\n`);
        //   }
        } else {
          // that.log(`兑换${rewardNum}京豆失败，原因：您目前只有${data.coin}积分，已不足兑换${giftValue}京豆所需的${salePrice}积分\n`)
      ...
      that.log(`${$.name}getExchangeRewards异常,${JSON.stringify($.getExchangeRewardsRes)}`)
    }
  } catch (e) {
    $.logErr(e)
  }
  ...
  return new Promise(resolve => {
    ...
    $.get({url: "https://a.jd.com//ajax/queryServerData.html",headers:{
      ...
      try {
        if (err) {
          that.log(`${JSON.stringify(err)}`)
          that.log(`${$.name} 获取京东服务器时间失败，请检查网路重试`)
        } else {
          data = JSON.parse(data);
          $.jdTime = data['serverTime'];
          // that.log(data['serverTime']);
          // that.log(data['serverTime'] - Date.now())
        }
      } catch (e) {
        $.logErr(e, resp)
      } finally {
...
async function get_diff_time() {
  // that.log(`本机时间戳 ${Date.now()}`)
  // that.log(`京东服务器时间戳 ${await getJDServerTime()}`)
