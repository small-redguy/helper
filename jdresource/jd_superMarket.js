...
async function help() {
  ...
  for (let code of newShareCodes) {
    ...
    that.log(`助力好友${JSON.stringify(res)}`);
  }
}
async function doDailyTask() {
  ...
  if (smtgQueryShopTaskRes.code === 0 && smtgQueryShopTaskRes.data.success) {
    ...
    for (let item of taskList) {
      that.log(` ${item['title'].length < 4 ? item['title']+`\xa0` : item['title'].slice(-4)}         ${item['finishNum'] === item['targetNum'] ? '已完成':'未完成'} ${item['finishNum']}/${item['targetNum']}`)
    }
    for (let item of taskList) {
      //领奖
      ...
      if ((item.type === 1 || item.type === 11) && item.taskStatus === 0) {
        ...
        that.log(`${item.subTitle}结果${JSON.stringify(res)}`)
      }
      if (item.type === 2) {
        //逛会场
...
async function receiveGoldCoin() {
  ...
  if ($.goldCoinData.data && $.goldCoinData.data.bizCode === 0) {
    that.log(`领取金币成功${$.goldCoinData.data.result.receivedGold}`)
    message += `【领取金币】${$.goldCoinData.data.result.receivedGold}个\n`;
  } else {
    that.log(`${$.goldCoinData.data && $.goldCoinData.data.bizMsg}`);
  }
}
        ...
        try {
          if (err) {
            ...
            if ($.data.data.bizCode !== 0 && $.data.data.bizCode !== 809) {
              ...
              that.log(`收取蓝币失败：${$.data.data.bizMsg}`)
              return
            }
            if  ($.data.data.bizCode === 0) {
              $.coincount += $.data.data.result.receivedBlue;
              $.blueCionTimes ++;
              that.log(`【京东账号${$.index}】${$.nickName} 第${$.blueCionTimes}次领蓝币成功，获得${$.data.data.result.receivedBlue}个\n`)
...
async function BeanSign() {
  ...
  if (beanSignRes && beanSignRes.data['bizCode'] === 0) {
    that.log(`每天从指定入口进入游戏,可获得额外奖励:${JSON.stringify(beanSignRes)}`)
  }
}
//每日签到
...
async function businessCircleActivity() {
  ...
  if (smtg_getTeamPkDetailInfoRes && smtg_getTeamPkDetailInfoRes.data.bizCode === 0) {
    ...
    that.log(`pkStatus:${pkStatus}\n`);
    that.log(`pkActivityId:${pkActivityId}\n`);
    ...
    if (joinStatus === 0) {
      if (joinPkTeam === 'true') {
        ...
        if ($.updatePkActivityIdRes && ($.updatePkActivityIdRes.pkActivityId === pkActivityId)) {
          ...
          if (res && res.data.bizCode === 0) {
            ...
            that.log(`加入战队失败,该战队已满\n无法加入`)
          } else {
            that.log(`加入战队其他未知情况:${JSON.stringify(res)}`)
          }
        } else {
      ...
      if (teamId) {
        that.log(`inviteCode: [${inviteCode}]`);
        that.log(`PK队伍teamId: [${teamId}]`);
        that.log(`PK队伍名称: [${currentUserPkInfo && currentUserPkInfo.teamName}]`);
        that.log(`我邀请的人数:${inviteCount}\n`)
        that.log(`\n我方战队战队 [${currentUserPkInfo && currentUserPkInfo.teamName}]/【${currentUserPkInfo && currentUserPkInfo.teamCount}】`);
        that.log(`对方战队战队 [${pkUserPkInfo && pkUserPkInfo.teamName}]/【${pkUserPkInfo && pkUserPkInfo.teamCount}】\n`);
      }
    }
    if (pkStatus === 1) {
      that.log(`商圈PK进行中\n`)
      if (!teamId) {
        ...
        that.log(`商圈PK奖励领取结果：${JSON.stringify(receivedPkTeamPrize)}\n`)
  ...
  if (businessCirclePKDetailRes && businessCirclePKDetailRes.data.bizCode === 0) {
    ...
    if (pkPrizeStatus === 2) {
      ...
      that.log(`商圈PK奖励领取结果：${JSON.stringify(getPkPrizeRes)}`)
    ...
    that.log(`我方商圈人气值/对方商圈人气值：${businessCircleVO.hotPoint}/${otherBusinessCircleVO.hotPoint}`);
    that.log(`我方商圈成员数量/对方商圈成员数量：${businessCircleVO.memberCount}/${otherBusinessCircleVO.memberCount}`);
    ...
    if (otherBusinessCircleVO.hotPoint - businessCircleVO.hotPoint > 300 && (Date.now() > (pkSettleTime - 24 * 60 * 60 * 1000))) {
      ...
      that.log(`商圈PK已过1天，对方商圈人气值还大于我方商圈人气值300，退出该商圈重新加入`);
      ...
      //退出该商圈
      if (inviteCode === '-4msulYas0O2JsRhE-2TA5XZmBQ') return;
      that.log(`商圈PK已过2天，对方商圈人气值还大于我方商圈人气值，退出该商圈重新加入`);
      await smtg_quitBusinessCircle();
    }
  } else if (businessCirclePKDetailRes && businessCirclePKDetailRes.data.bizCode === 222) {
    that.log(`${businessCirclePKDetailRes.data.bizMsg}`);
    that.log(`开始领取商圈PK奖励`);
    const getPkPrizeRes = await smtg_getPkPrize();
    that.log(`商圈PK奖励领取结果：${JSON.stringify(getPkPrizeRes)}`)
    ...
    that.log(`您暂未加入商圈,现在给您加入LXK9301的商圈`);
    const joinBusinessCircleRes = await smtg_joinBusinessCircle(myCircleId);
    that.log(`参加商圈结果：${JSON.stringify(joinBusinessCircleRes)}`)
    if (joinBusinessCircleRes.data.bizCode !== 0) {
      that.log(`您加入LXK9301的商圈失败，现在给您随机加入一个商圈`);
      ...
      if (BusinessCircleList.data.bizCode === 0) {
        ...
        that.log(`随机加入商圈结果：${JSON.stringify(joinBusinessCircleRes)}`)
      }
    }
  } else {
    that.log(`访问商圈详情失败：${JSON.stringify(businessCirclePKDetailRes)}`);
  }
}
//我的货架
async function myProductList() {
  ...
  if (shelfListRes.data.bizCode === 0) {
    ...
    for (let item of shelfList) {
      ...
      that.log(`货架等级 level ${item.level}/${item.maxLevel}`);
      that.log(`上架状态 groundStatus ${item.groundStatus}`);
      that.log(`解锁状态 unlockStatus ${item.unlockStatus}`);
      that.log(`升级状态 upgradeStatus ${item.upgradeStatus}`);
      if (item.unlockStatus === 0) {
        that.log(`${item.name}不可解锁`)
      } else if (item.unlockStatus === 1) {
        that.log(`${item.name}可解锁`);
        await smtg_unlockShelf(item.shelfId);
      } else if (item.unlockStatus === 2) {
        that.log(`${item.name}已经解锁`)
      }
      if (item.groundStatus === 1) {
        that.log(`${item.name}可上架`);
        ...
        if (item.productInfo.productType === 2) {
          that.log(`[${item.name}][限时商品]`)
        } else if (item.productInfo.productType === 1){
          that.log(`[${item.name}]`)
        } else {
          that.log(`[${item.name}][productType:${item.productInfo.productType}]`)
        }
      }
    }
  }
...
async function upgrade() {
  ...
  that.log('目前没有平稳升级,只取倒数几个商品进行升级,普通货架取倒数4个商品,冰柜货架取倒数3个商品,水果货架取倒数2个商品')
  const smtgProductListRes = await smtg_productList();
  if (smtgProductListRes.data.bizCode === 0) {
    ...
    for (let item of shelfCategorys) {
      that.log(`  ${item["name"].length<3?item["name"]+`\xa0`:item["name"]}       ${item['shelfCategory'] === 1 ? '普通货架' : item['shelfCategory'] === 2 ? '冰柜货架' : item['shelfCategory'] === 3 ? '水果货架':'未知货架'}       ${item["unlockStatus"] === 0 ? '---' : item["level"]+'级'}     ${item["unlockStatus"] === 0 ? '未解锁' : '已解锁'}      ${item["upgradeStatus"] === 1 ? '可以升级' : item["upgradeStatus"] === 0 ? '不可升级':item["upgradeStatus"]}`)
    }
  ...
  if (shelfListRes.data.bizCode === 0) {
    ...
    that.log(`待升级货架数量${shelfList_upgrade.length}个`);
    if (shelfList_upgrade && shelfList_upgrade.length > 0) {
      shelfList_upgrade.sort(sortSyData);
      that.log("\n可升级货架名         等级     升级所需金币");
      for (let item of shelfList_upgrade) {
        that.log(` [${item["name"]}]         ${item["level"]}/${item["maxLevel"]}         ${item["upgradeCostGold"]}`);
      }
      that.log(`开始升级[${shelfList_upgrade[0].name}]货架，当前等级${shelfList_upgrade[0].level}，所需金币${shelfList_upgrade[0].upgradeCostGold}\n`);
      await smtg_upgradeShelf(shelfList_upgrade[0].shelfId);
    }
  }
}
async function manageProduct() {
  that.log(`安排上货(单价最大商品)`);
  const shelfListRes = await smtg_shelfList();
  if (shelfListRes.data.bizCode === 0) {
    const { shelfList } = shelfListRes.data.result;
    that.log(`我的货架数量:${shelfList && shelfList.length}`);
              ...
              for (let item of taskList) {
                if (item.taskStatus === 1) {
                  if (item.prizeStatus === 1) {
                    ...
                    that.log(`[${item.title}] 已做完 ${item.finishNum}/${item.targetNum}`);
                  }
                } else {
                  that.log(`[${item.title}] 未做完 ${item.finishNum}/${item.targetNum}`)
                  if (item.content) {
                    const { itemId } = item.content[item.type];
                    that.log('itemId', itemId)
...
function smtg_shelfProductList(shelfId) {
  return new Promise((resolve) => {
    that.log(`开始检查货架[${shelfId}] 可上架产品`)
    $.get(taskUrl('smtg_shelfProductList', { shelfId }), (err, resp, data) => {
      try {
        // that.log(`检查货架[${shelfId}] 可上架产品结果:${data}`)
        if (err) {
          that.log('\n东东超市: API查询请求失败 ‼️‼️')
          that.log(JSON.stringify(err));
        } else {
...
function smtg_upgradeProduct(productId) {
  return new Promise((resolve) => {
    $.get(taskUrl('smtg_upgradeProduct', { productId }), (err, resp, data) => {
      try {
        // that.log(`升级商品productId[${productId}]结果:${data}`);
        if (err) {
          that.log('\n东东超市: API查询请求失败 ‼️‼️')
          that.log(JSON.stringify(err));
        } else {
...
function smtg_unlockProduct(productId) {
  return new Promise((resolve) => {
    ...
    $.get(taskUrl('smtg_unlockProduct', { productId }), (err, resp, data) => {
      try {
        // that.log(`解锁商品productId[${productId}]结果:${data}`);
        if (err) {
          that.log('\n东东超市: API查询请求失败 ‼️‼️')
          that.log(JSON.stringify(err));
        } else {
...
function smtg_upgradeShelf(shelfId) {
  return new Promise((resolve) => {
    $.get(taskUrl('smtg_upgradeShelf', { shelfId }), (err, resp, data) => {
      try {
        // that.log(`升级货架shelfId[${shelfId}]结果:${data}`);
        if (err) {
          that.log('\n东东超市: API查询请求失败 ‼️‼️')
          that.log(JSON.stringify(err));
        } else {
...
function smtg_unlockShelf(shelfId) {
  return new Promise((resolve) => {
    ...
    $.get(taskUrl('smtg_unlockShelf', { shelfId }), (err, resp, data) => {
      try {
        // that.log(`解锁货架shelfId[${shelfId}]结果:${data}`);
        if (err) {
          that.log('\n东东超市: API查询请求失败 ‼️‼️')
          that.log(JSON.stringify(err));
        } else {
...
function shareCodesFormat() {
  return new Promise(resolve => {
    that.log(`第${$.index}个京东账号的助力码:::${jdSuperMarketShareArr[$.index - 1]}`)
    if (jdSuperMarketShareArr[$.index - 1]) {
      newShareCodes = jdSuperMarketShareArr[$.index - 1].split('@');
    } else {
      that.log(`由于您未提供与京京东账号相对应的shareCode,下面助力将采纳本脚本自带的助力码\n`)
      const tempIndex = $.index > shareCodes.length ? (shareCodes.length - 1) : ($.index - 1);
      newShareCodes = shareCodes[tempIndex].split('@');
    }
    that.log(`格式化后第${$.index}个京东账号的助力码${JSON.stringify(newShareCodes)}`)
    resolve();
  })
}
function requireConfig() {
  return new Promise(resolve => {
    ...
    // that.log(`东东超市已改版,目前暂不用助力, 故无助力码`)
    // that.log(`\n东东超市商圈助力码::${JSON.stringify(jdSuperMarketShareArr)}`);
    // that.log(`您提供了${jdSuperMarketShareArr.length}个账号的助力码\n`);
    resolve()
  })
}
function TotalBean() {
  ...
  return new Promise(async resolve => {
    ...
    $.get({url: `http://jd.turinglabs.net/api/v2/jd/supermarket/read/100000/`, timeout: 100000}, (err, resp, data) => {
      try {
        if (err) {
          that.log(`${JSON.stringify(err)}`)
          that.log(`${$.name} supermarket/read/ API请求失败，请检查网路重试`)
        } else {
          data = JSON.parse(data);
