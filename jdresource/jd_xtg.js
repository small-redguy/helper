...
!(async () => {
  ...
  for (let i = 0; i < cookiesArr.length; i++) {
    if (cookiesArr[i]) {
      ...
      that.log(`一共${starID.length}个${$.name}任务，耗时会很久，请提前知晓`);
  ...
  //     for (let index = 0; index < starID.length; index++) {
  //       $.activeId = starID[index]['starId'];
  //       that.log(`账号${v + 1}去助力 账号${Number(item) + 1} 的${$.activeId}活动的邀请码${$.allShareId[item][index]}\n`)
  //       await doSupport($.allShareId[item][index]);
  //     }
  //   }
  //   that.log(`如有剩下的机会，助力作者\n\n`);
  //   for (let index = 0; index < starID.length; index++) {
  //     $.activeId = starID[index]['starId'];
