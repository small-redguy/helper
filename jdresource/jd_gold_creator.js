    ...
    $.get(options, async (err, resp, data) => {
      try {
        if (err) {
          ...
          that.log(`${$.name} goldCreatorDetail API请求失败，请检查网路重试`)
        } else {
          if (safeGet(data)) {
            ...
            if (data.code === '0') {
              ...
              that.log(`共有${$.subTitleInfos.length}个主题`);
              ...
              $.subTitleInfos = $.subTitleInfos.filter(vo => !!vo && vo['hasVoted'] === '0');
              that.log(`已投票${unVoted - $.subTitleInfos.length}主题\n`);
            } else {
              that.log(`goldCreatorTab 异常：${JSON.stringify(data)}`)
            }
          }
        }
      } catch (e) {
          ...
          if (safeGet(data)) {
            ...
            if (data.code === '0') {
              ...
              if (flag) {
                ...
                that.log(`当前剩余投票次数：${$.remainVotes}`);
                await doTask(subTitleId, taskId, batchId);
              }
            } else {
              that.log(`goldCreatorDetail 异常：${JSON.stringify(data)}`)
            }
          }
        }
      } catch (e) {
...
async function doTask(subTitleId, taskId, batchId) {
  ...
  that.log(`给 【${$.skuList[randIndex]['name']}】 商品投票`);
  const body = {
    "stageId": $.stageId,
    subTitleId,
          ...
          if (safeGet(data)) {
            ...
            if (data.code === '0') {
              if (data.result.taskCode === '0') {
                that.log(`成功，获得 ${data.result.lotteryScore}京豆\n`);
                if (data.result.lotteryScore) $.beans += parseInt(data.result.lotteryScore);
              } else {
                that.log(`失败：${data.result['taskMsg']}\n`);
              }
            } else {
