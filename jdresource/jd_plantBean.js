...
async function doGetReward() {
	...
	if (roundList[num].dateDesc.indexOf('本期 ') > -1) {
		roundList[num].dateDesc = roundList[num].dateDesc.substr(roundList[num].dateDesc.indexOf('本期 ') + 3,
			roundList[num].dateDesc.length);
	}
...
async function stealFriendWater() {
	...
	if ($.stealFriendList && $.stealFriendList.code === '0') {
		...
		if ($.stealFriendList.data && $.stealFriendList.data.friendInfoList && $.stealFriendList.data.friendInfoList
			.length > 0) {
			let nowTimes = new Date(new Date().getTime() + new Date().getTimezoneOffset() * 60 * 1000 + 8 * 60 *
				60 * 1000);
...
function showTaskProcess() {
	return new Promise(async resolve => {
		...
		if ($.taskList && $.taskList.length > 0) {
			...
			for (let item of $.taskList) {
				console.log(
					`[${item["taskName"]}]  ${item["gainedNum"]}/${item["totalNum"]}   ${item["isFinished"]}`
					);
			}
		}
		resolve()
...
function getHelp() {
	return new Promise(resolve => {
	    newShareCodes=[];
		$.get({
			url: "http://api.tyh52.com/act/get/plantBean/3"
		}, (err, resp, data) => {
			try {
				if (data) {
...
function setHelp() {
	return new Promise(resolve => {
		if ($.myPlantUuid) {
			$.get({
				url: "http://api.tyh52.com/act/set/plantBean/" + $.myPlantUuid
			}, (err, resp, data) => {
				try {
					if (data) {
						data = JSON.parse(data);
...
async function plantShareSupportList() {
	...
	if ($.shareSupportList && $.shareSupportList.code === '0') {
		...
		//次日北京时间0点时间戳
		const UTC8_End_Time = parseInt((Date.now() + 28800000) / 86400000) * 86400000 - 28800000 + (24 * 60 * 60 *
			1000);
		let friendList = [];
		data.map(item => {
