...
async function jdFriend() {
  ...
  that.log(`${$.name} 任务运行完成，共计获得 ${$.bean} 京豆`)
}
      ...
      try {
        if (err) {
          ...
          if (safeGet(data)) {
            data = JSON.parse(data);
            if (data.code === '0') {
              that.log(`与好友【${unionPin}】匹配成功，匹配程度：${data.data.intimacy.total}`)
      ...
      try {
        if (err) {
          ...
          if (safeGet(data)) {
            data = JSON.parse(data);
            if (data.code === '0') {
              that.log(`打开宝箱成功，获得${data.data.bean ? data.data.bean : 0}京豆`)
...
function taskGetUrl(function_id, body) {
  //that.log(`https://api.m.jd.com/?appid=joy-friend&functionId=${function_id}&body=${escape(JSON.stringify(body))}`)
  return {
